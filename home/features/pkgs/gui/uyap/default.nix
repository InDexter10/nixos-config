...
let
  ...
  uyap-launcher = pkgs.writeShellScriptBin "uyap-editor" ''
        ...
        if [ -d "/run/current-system/sw/share/X11/fonts" ]; then
            ...
            EXTRA_BINDS="$EXTRA_BINDS --ro-bind-try /run/current-system/sw/share/fonts /run/current-system/sw/share/fonts"
    EXTRA_BINDS="$EXTRA_BINDS --ro-bind-try /var/cache/fontconfig /var/cache/fontconfig"
        fi
